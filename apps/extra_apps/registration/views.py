...
def activate(request, backend,
    ...
    **Template:**
    ...
    ## backend.activate will active the user 
...
@csrf_exempt
def register(request, backend, identity, success_url=None, form_class=None,
    ...
    **Template:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ## backend.register will send a email.  
            new_user = backend.register(request, **form.cleaned_data)

            BaseObj=Base.objects.get(user=new_user)
            BaseObj.identity=identity
            BaseObj.save()
            if identity == "D":
                DentistProfileNew = DentistProfile(user = BaseObj)
                DentistProfileNew.save()
            else:
                PatientProfileNew = PatientProfile(user = BaseObj)
                PatientProfileNew.save()
            
            ## need to verify the user's info, and auto login
            if success_url is None:
